...
public class DapperBookRepository(string connectionString) : IBookRepository
{
    ...
    public async Task<Guid> AddAsync(string title, string author, CancellationToken ct)
    {
        var id = Guid.NewGuid();
        using var conn = Connect();
        await conn.ExecuteAsync(
            "INSERT INTO Books (Id, Title, Author) VALUES (@Id, @Title, @Author)",
            new { Id = id.ToString(), Title = title, Author = author });
        return id;
    }
    ...
    public async Task<bool> UpdateAsync(Guid id, string title, string author, CancellationToken ct)
    {
        using var conn = Connect();
